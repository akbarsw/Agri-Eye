...
const CONNECTIONS = [
  { from: POINTS[0], to: SOLO },
  { from: POINTS[1], to: SOLO },
  { from: POINTS[2], to: SOLO },
  { from: POINTS[3], to: SOLO },
];
      ...
      <div className="relative mx-auto max-w-4xl px-3">
        ...
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-[10px] text-slate-500">Monitoring data panen aktif • Jawa Tengah</p>
          </div>
          <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 text-[10px] font-semibold">
        ...
        >
          <div className="grid lg:grid-cols-5 gap-0">
            ...
            {/* Left: Cards */}
            <div className="lg:col-span-2 p-3 lg:p-4 flex flex-col gap-2 relative z-10">
              ...
              <motion.div
                ...
                className="rounded-lg p-3 border backdrop-blur-md"
                style={{ background: "rgba(255,255,255,0.85)", borderColor: "#86efac" }}
              >
                <div className="flex items-center gap-1.5 mb-1.5">
                  <MapPin className="w-3 h-3 text-emerald-600" />
                  <span className="text-[9px] font-semibold text-emerald-600 tracking-widest uppercase">Area Prioritas</span>
                </div>
                <h4 className="text-emerald-900 font-bold text-sm">Jawa Tengah</h4>
                <p className="text-[10px] text-slate-500">7 kabupaten • Solo Raya</p>
                <div className="flex gap-1.5 mt-2">
                  <span className="px-1.5 py-0.5 rounded-full text-[9px] font-semibold bg-emerald-100 text-emerald-700">342 petani</span>
                  <span className="px-1.5 py-0.5 rounded-full text-[9px] font-semibold bg-sky-100 text-sky-700">128 pembeli</span>
                </div>
              </motion.div>
              ...
              >
                <div className="flex items-center gap-1.5 mb-1.5">
                  ...
                  <span className="text-[9px] font-semibold text-emerald-600 tracking-widest uppercase">Data Panen Terbaru</span>
                </div>
                <div className="flex items-start justify-between">
                  <div>
                    <h4 className="text-emerald-900 font-bold text-sm">Cabai Merah</h4>
                    <p className="text-[10px] text-slate-500">Boyolali • 100 kg • Grade A</p>
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[9px] font-semibold text-emerald-600 tracking-widest uppercase">Skor Kesiapan Jual</span>
                  </div>
                  <span className="text-[10px] font-bold text-emerald-600">+12%</span>
                </div>
                <div className="flex items-end gap-1.5">
                  <span className="text-2xl font-bold text-emerald-900">87%</span>
                  <span className="text-[10px] text-slate-500 mb-0.5">siap ekspor</span>
                </div>
            ...
            {/* Right: Map */}
            <div className="lg:col-span-3 relative p-2 lg:p-3 flex items-center justify-center">
              <motion.div
              ...
              >
                <svg viewBox="250 120 260 210" className="w-full h-full" style={{ minHeight: "280px" }}>
                  <defs>
                    <filter id="lineGlow">
                  ...
                  {Object.entries(KABUPATEN_PATHS).map(([name, d]) => (
                    <path
                      key={name}
                      d={d}
                      fill={name === "Surakarta" ? "#34d399" : "#a7f3d0"}
                      stroke="#34d399"
                      strokeWidth="0.5"
                      opacity="0.7"
                    />
                  ...
                  />
                  <text x={SOLO.x + 10} y={SOLO.y - 6} fill="#92400e" fontSize="7" fontWeight="bold">Solo</text>
                  ...
                  {POINTS.map((p, i) => (
                    <g key={i} onMouseEnter={() => setHoveredPoint(i)} onMouseLeave={() => setHoveredPoint(null)} style={{ cursor: "pointer" }}>
                      ...
                      />
                      <text x={p.x + 6} y={p.y + 2} fill="#374151" fontSize="6" fontWeight="500">{p.name}</text>
                      ...
                      {hoveredPoint === i && (
                        <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                          <rect
                            ...
                            strokeWidth="0.6"
                          />
                          <text x={p.x - 42} y={p.y - 27} fill="#065f46" fontSize="7" fontWeight="bold">{p.crop} • {p.grade}</text>
                          <text x={p.x - 42} y={p.y - 17} fill="#6b7280" fontSize="6">{p.yield} • {p.name}</text>
                          <text x={p.x - 42} y={p.y - 8} fontSize="6" fill={p.status === "verified" ? "#059669" : "#d97706"}>
