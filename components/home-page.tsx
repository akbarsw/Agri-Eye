...
const features = [
  { title: 'Catat panen secara digital', text: 'Petani mencatat komoditas, jumlah, tanggal panen, lokasi asal, foto produk, dan harga harapan dalam satu dashboard sederhana.', icon: Sprout },
  { title: 'QR Produk per Data Panen', text: 'Setiap Data Panen yang lolos validasi mendapatkan QR Produk berisi asal produk, grade mutu, dan riwayat distribusi.', icon: QrCode },
  { title: 'Marketplace B2B berbasis data', text: 'Produk terverifikasi dapat ditawarkan ke restoran, hotel, katering, UMKM pangan, toko modern, dan distributor.', icon: PackageCheck },
];
    ...
    <main className="min-h-screen bg-[#F7F4ED] text-slate-900 antialiased">
      <nav className="sticky top-0 z-50 border-b border-[#E8DDC7]/70 bg-[#F7F4ED]/85 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-2 sm:px-6 lg:px-8">
          ...
          <div className="flex shrink-0 items-center"><AgriLoginSelect /></div>
        </div>
      ...
      <section id="top" className="relative overflow-hidden px-6 py-4 lg:px-8 lg:py-6">
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-7 lg:grid-cols-[0.9fr_1.1fr]">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-[#E8DDC7] bg-white/80 px-4 py-2 text-sm font-medium text-[#1FA653] shadow-sm"><CheckCircle2 className="h-4 w-4" />Food traceability untuk hasil panen lokal</div>
            <AgriTypewriter />
            <p className="mt-5 max-w-2xl text-base leading-7 text-slate-500 lg:text-lg lg:leading-8">AGRI-EYE membantu petani mencatat Data Panen, membuat QR Produk, dan menjual produk terverifikasi ke pembeli B2B secara lebih transparan.</p>
            <div className="mt-6 flex flex-col gap-3 sm:flex-row">
              <a href="/farmer" className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[#25B866] px-6 py-4 text-sm font-bold !text-white shadow-sm transition-all duration-200 ease-in-out hover:bg-[#1FA653] hover:shadow-md">Mulai sebagai Petani<ArrowRight className="h-4 w-4" /></a>
              <a href="/buyer" className="inline-flex items-center justify-center rounded-2xl border border-[#E8DDC7] bg-white/80 px-6 py-4 text-sm font-bold text-slate-700 shadow-sm transition-all duration-200 ease-in-out hover:bg-white hover:shadow-md">Masuk sebagai Pembeli B2B</a>
            </div>
            <div className="mt-7 grid max-w-xl grid-cols-3 gap-3">
              {stats.map((item) => (
                <div key={item.label} className="rounded-2xl border border-[#E8DDC7] bg-white/80 p-4 shadow-sm backdrop-blur transition-all duration-200 ease-in-out hover:border-[#BFE8CC] hover:bg-white hover:shadow-md"><p className="overflow-hidden text-xl font-bold tracking-tight text-slate-900 lg:text-2xl"><AgriAnimatedNumber value={item.value} /></p><p className="mt-1.5 text-xs font-medium leading-5 text-slate-500">{item.label}</p></div>
              ))}
            </div>
          </div>
          <AgriTraceMap />
      ...
      <section id="platform" className="px-6 py-4 lg:px-8 lg:py-5">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-3">
            {features.map((feature) => { const Icon = feature.icon; return <article key={feature.title} className="rounded-[1.5rem] border border-[#E8DDC7] bg-white/85 p-6 shadow-sm backdrop-blur transition-all duration-200 ease-in-out hover:-translate-y-1 hover:border-[#BFE8CC] hover:bg-white hover:shadow-md"><div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#EAF8F0] text-[#25B866]"><Icon className="h-5 w-5" /></div><h3 className="mt-5 text-lg font-bold tracking-tight text-slate-900">{feature.title}</h3><p className="mt-3 text-sm leading-6 text-slate-500">{feature.text}</p></article>; })}
          </div>
        </div>
      </section>
